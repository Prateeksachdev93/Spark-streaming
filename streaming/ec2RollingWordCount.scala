 ...
 */
object ec2RollingWordCount {
  def main(args: Array[String]) {
    if (args.length < 2) {
      System.err.println("Usage: ec2RollingWordCount <hostname> <port>")
      System.exit(1)
    }
   ...
   StreamingExamples.setStreamingLogLevels()
    ...
    val ssc = new StreamingContext("spark://ec2-107-20-106-130.compute-1.amazonaws.com:7077","ec2RollingWordCount", Seconds(60*15))
    ...
    val sortedcount = refmap.reduceByKeyAndWindow(_ + _, Seconds(60*15))
    ...
    val sortedcountslide = slidemap.reduceByKeyAndWindow(_ + _, Seconds(60*15))
                     .map{case (topic, count) => (count, topic)}
                     .transform(_.sortByKey(false))
